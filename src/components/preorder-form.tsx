...
export function PreorderForm() {
  ...
  const firstPickupRef = useRef<HTMLButtonElement>(null);
  ...
  async function submit(event: React.FormEvent<HTMLFormElement>) {
    ...
    if (Object.keys(nextErrors).length > 0) {
      ...
      setStatus("idle");
      const firstInvalid = nextErrors.pickup ? firstPickupRef : nextErrors.name ? nameRef : phoneRef;
  ...
  return (
    <div className="grid gap-4 lg:grid-cols-[.9fr_1.1fr]">
      ...
      <form onSubmit={submit} className="rounded-[30px] border border-ink/14 bg-white/35 p-5 sm:p-8" noValidate>
        ...
        <fieldset className="mt-8"><legend className="text-sm font-extrabold">Время выдачи</legend><div className="mt-3 grid grid-cols-2 gap-2">{pickupOptions.map((option, index) => <button key={option} ref={index === 0 ? firstPickupRef : undefined} type="button" onClick={() => update("pickup", option)} aria-pressed={values.pickup === option} aria-describedby={errors.pickup ? "pickup-error" : undefined} className={`min-h-12 rounded-2xl border px-3 text-left text-xs font-bold transition-colors ${values.pickup === option ? "border-cobalt bg-cobalt text-white" : "border-ink/14 hover:border-ink"}`}>{option}</button>)}</div>{errors.pickup && <p id="pickup-error" className="mt-2 text-xs font-bold text-[#B42518]" role="alert">{errors.pickup}</p>}</fieldset>
        <div className="mt-7 grid gap-5">
          <label className="grid gap-2 text-sm font-extrabold">Имя<input ref={nameRef} value={values.name} onChange={(event) => update("name", event.target.value)} className="min-h-13 rounded-2xl border border-ink/18 bg-oyster px-4 outline-none transition-colors focus:border-cobalt" placeholder="Артём" autoComplete="given-name" aria-invalid={Boolean(errors.name)} aria-describedby={errors.name ? "name-error" : undefined} />{errors.name && <span id="name-error" className="text-xs text-[#B42518]" role="alert">{errors.name}</span>}</label>
          <label className="grid gap-2 text-sm font-extrabold">Телефон<input ref={phoneRef} value={values.phone} onChange={(event) => update("phone", event.target.value)} className="min-h-13 rounded-2xl border border-ink/18 bg-oyster px-4 outline-none transition-colors focus:border-cobalt" placeholder="+7 999 000-00-00" inputMode="tel" autoComplete="tel" aria-invalid={Boolean(errors.phone)} aria-describedby={errors.phone ? "phone-error" : "phone-note"} /><span id="phone-note" className="text-[11px] font-medium text-ink/65">В реальном сервисе — только для статуса заказа.</span>{errors.phone && <span id="phone-error" className="text-xs text-[#B42518]" role="alert">{errors.phone}</span>}</label>
        </div>
